...
transacciones_totales = df['Invoice ID'].nunique()
...
col2.metric("Transacciones Totales", transacciones_totales)
...
with col6:
    ...
    scatter = ax10.scatter(df_filtered["cogs"], df_filtered["gross income"], df_filtered["Unit price"],
                           c=df_filtered["Rating"], cmap="viridis")
...
fig8 = px.sunburst(sunburst_df, path=['Branch', 'Product line'], values='gross income', title="Ingreso Bruto por Sucursal y Línea de Producto")
